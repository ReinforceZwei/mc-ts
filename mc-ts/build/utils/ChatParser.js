...
function parseChat(chatObj, parentState) {
    function getColorize(parentState) {
        let myColor = '';
        if ('color' in parentState)
            myColor += colors[parentState.color] + '+';
        if (parentState.bold)
            myColor += 'bold+';
        if (parentState.underlined)
            myColor += 'underline+';
        if (parentState.obfuscated)
            myColor += 'obfuscated+';
        if (myColor.length > 0)
    ...
    if (typeof chatObj === 'string') {
        ...
        let chat = '';
        if ('color' in chatObj)
            parentState.color = chatObj.color;
        if ('bold' in chatObj)
            parentState.bold = chatObj.bold;
        if ('italic' in chatObj)
            parentState.italic = chatObj.italic;
